...
// The posts either side of this one in the list's order, where they exist.
const neighbours = (blog: BlogPost[], post: BlogPost | undefined) => {
  const index = post ? blog.findIndex((p) => p.id === post.id) : -1;
  return {
    newer: index > 0 ? blog[index - 1] : null,
    older: index >= 0 && index < blog.length - 1 ? blog[index + 1] : null,
  };
};
...
// The cover, when there is one, then the title block.
const PostLead = ({ post }: { post: BlogPost }) => (
  <>
    {post.cover && (
...
// The piece's canonical home is elsewhere, so show the summary if one exists
// and then hand the reader off instead of mirroring the text.
const OffSiteBody = ({ post, externalUrl }: { post: BlogPost; externalUrl: string }) => (
    ...
    >
      <span>
        ...
        <span className="font-serif text-lg leading-snug transition-colors group-hover/out:text-signal">
          This piece lives on {hostLabel(externalUrl) ?? "another site"}. Read it there.
        </span>
      </span>
      <ArrowUpRight
        size={20}
...
const PostBody = ({ post }: { post: BlogPost }) =>
  post.externalUrl ? (
    <OffSiteBody post={post} externalUrl={post.externalUrl} />
  ) : (
    <div className="prose dark:prose-invert prose-essay mx-auto">
      <Markdown>
        {post.body || ""}
...
const NeighbourLinks = ({ newer, older }: { newer: BlogPost | null; older: BlogPost | null }) =>
  newer || older ? (
    <nav
...
export const BlogPostPage = () => {
  ...
  const { newer, older } = neighbours(blog, post);
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      >
        ...
        <NeighbourLinks newer={newer} older={older} />
      </m.article>
    </div>
  );
};
